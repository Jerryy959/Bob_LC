...
function graphqlRequest(endpoint, query, variables, cancelSignal, timeoutSeconds, debug, proxy) {
  return $http.request({
    method: 'POST',
    url: endpoint,
    header: {
...
function searchProblems(keyword, cancelSignal, timeoutSeconds, maxCandidates, debug, proxy) {
  ...
  return graphqlRequest(endpoint, query, { search: keyword, limit: maxCandidates || 3 }, cancelSignal, timeoutSeconds, debug, proxy).then(function (data) {
    var list = (((data || {}).data || {}).problemsetQuestionList || {}).questions || [];
    return { endpoint: endpoint, list: list };
  });
}

function fetchProblemDetails(slug, cancelSignal, timeoutSeconds, debug, endpointOverride, proxy) {
  var endpoint = endpointOverride || pickEndpoint(slug);
  var query = `query questionData($titleSlug: String!) {\n  question(titleSlug: $titleSlug) {\n    title\n    titleSlug\n    content\n    translatedTitle\n    translatedContent\n    difficulty\n    exampleTestcases\n    similarQuestions\n    topicTags { name slug }\n    constraints\n  }\n}`;
  return graphqlRequest(endpoint, query, { titleSlug: slug }, cancelSignal, timeoutSeconds, debug, proxy).then(function (data) {
...
function resolveById(id, cancelSignal, timeoutSeconds, debug, proxy) {
  ...
  return graphqlRequest(endpoint, query, { search: id }, cancelSignal, timeoutSeconds, debug, proxy).catch(function () {
    endpoint = CN_ENDPOINT;
    return graphqlRequest(endpoint, query, { search: id }, cancelSignal, timeoutSeconds, debug, proxy);
  }).then(function (data) {
    var list = (((data || {}).data || {}).problemsetQuestionList || {}).questions || [];
