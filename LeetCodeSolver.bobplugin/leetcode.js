...
function graphqlRequest(endpoint, query, variables, cancelSignal, timeoutSeconds, debug) {
  return $http.request({
    method: 'POST',
    url: endpoint,
    header: {
      'Content-Type': 'application/json',
      'Referer': endpoint.indexOf('leetcode.cn') !== -1 ? 'https://leetcode.cn' : 'https://leetcode.com',
    ...
    throw util.buildError('network', 'LeetCode 接口返回异常', 'status: ' + resp.statusCode + ', body: ' + JSON.stringify(resp.data || resp.body));
  });
}
...
function searchProblems(keyword, cancelSignal, timeoutSeconds, maxCandidates, debug) {
  ...
  return graphqlRequest(endpoint, query, { search: keyword, limit: maxCandidates || 3 }, cancelSignal, timeoutSeconds, debug).then(function (data) {
    var list = (((data || {}).data || {}).problemsetQuestionList || {}).questions || [];
    return { endpoint: endpoint, list: list };
  });
}

function fetchProblemDetails(slug, cancelSignal, timeoutSeconds, debug, endpointOverride) {
  var endpoint = endpointOverride || pickEndpoint(slug);
  var query = `query questionData($titleSlug: String!) {\n  question(titleSlug: $titleSlug) {\n    title\n    titleSlug\n    content\n    translatedTitle\n    translatedContent\n    difficulty\n    exampleTestcases\n    similarQuestions\n    topicTags { name slug }\n    constraints\n  }\n}`;
  return graphqlRequest(endpoint, query, { titleSlug: slug }, cancelSignal, timeoutSeconds, debug).then(function (data) {
...
function resolveById(id, cancelSignal, timeoutSeconds, debug) {
  ...
  return graphqlRequest(endpoint, query, { search: id }, cancelSignal, timeoutSeconds, debug).catch(function () {
    endpoint = CN_ENDPOINT;
    return graphqlRequest(endpoint, query, { search: id }, cancelSignal, timeoutSeconds, debug);
  }).then(function (data) {
    var list = (((data || {}).data || {}).problemsetQuestionList || {}).questions || [];
